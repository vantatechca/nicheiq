...
export default function SettingsPage() {
  ...
  async function exportEverythingJson() {
    ...
    try {
      ...
      downloadJson(`nicheiq-export-${today}.json`, {
        opportunities,
        products,
        creators,
        signals,
  ...
  async function exportCreatorsCsv() {
    setExporting("creators");
    try {
  ...
  return (
    <>
      <PageHeader
        title="Settings"
        description="Profile, AI preferences, notifications, data export."
      />
          ...
          <Card className="border-slate-800 bg-slate-900/40">
            ...
            <CardContent className="grid gap-3 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <Button onClick={() => toast.info("Profile editing not wired yet")}>Save</Button>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="ai">
          <Card className="border-slate-800 bg-slate-900/40">
            <CardHeader>
              ...
              <CardDescription>
                Pick a model per task tier. Tier 3 spending is capped daily.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {[
                {
                  tier: "Tier 1 (bulk classify)",
                  default: "openrouter/qwen-2.5",
                  note: "≤ $0.10 / 1M tokens",
                },
                {
                  tier: "Tier 2 (analysis)",
                  default: "anthropic/claude-haiku-4.5",
                  note: "Per-product enrichment",
                },
                {
                  tier: "Tier 3 (strategic)",
                  default: "anthropic/claude-sonnet-4",
                  note: "Brain chat + digests · capped at $5/day",
                },
              ].map((t) => (
                <div
                  key={t.tier}
                  className="flex items-center justify-between rounded-md border border-slate-800 bg-slate-950/40 p-3"
                >
                  <div>
              ...
              <div className="flex items-center justify-between rounded-md border border-slate-800 bg-slate-950/40 p-3">
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    Refuses tier-3 calls above this amount.
                  </div>
                </div>
                <Input
                  type="number"
                  defaultValue={5}
                  className="w-28 border-slate-800 bg-slate-900"
                />
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="data">
          <Card className="border-slate-800 bg-slate-900/40">
            <CardHeader>
              ...
              <CardDescription>
                Download a snapshot of your tracked entities. Exports pull from your live Neon
                database. CSV is best for spreadsheets, JSON keeps nested fields intact (score
                breakdowns, AI build plans, etc).
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                >
                  {exporting === "json" ? "Exporting…" : "Export everything (.json)"}
                </Button>
                <Button
                  variant="outline"
                  disabled={exporting !== null}
              ...
              <div className="mt-3 rounded-md border border-slate-800 bg-slate-950/40 p-3 text-xs text-slate-400">
                Exports include only what&apos;s currently in your dataset, fetched live from the
                database when you click.
              </div>
            </CardContent>
          </Card>
        </TabsContent>
