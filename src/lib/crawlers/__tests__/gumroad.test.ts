...
describe("estimateFromSalesAmortized (Gumroad — real sales, no date)", () => {
  it("spreads lifetime sales over the default 12-month window", () => {
    // 240 lifetime sales / 12 = 20/mo central; low = 60% = 12.
    const est = estimateFromSalesAmortized({ totalSales: 240, priceUsd: 25 });
    expect(est.sales.high).toBe(20);
    expect(est.sales.low).toBe(12);
    expect(est.revenue.high).toBe(20 * 25);
    expect(est.revenue.low).toBe(12 * 25);
    expect(est.basis).toBe("sales-amortized");
  });

  it("honours a custom window and clamps it to at least one month", () => {
    expect(estimateFromSalesAmortized({ totalSales: 120, priceUsd: 10, windowMonths: 6 }).sales.high).toBe(20);
    expect(estimateFromSalesAmortized({ totalSales: 50, priceUsd: 10, windowMonths: 0 }).sales.high).toBe(50);
  });

  it("tags as sales-amortized (distinct from sales-derived) and is readable", () => {
    const est = estimateFromSalesAmortized({ totalSales: 100, priceUsd: 5 });
    expect(readRevenueBasis([revenueBasisTag(est.basis)])).toBe("sales-amortized");
  });
});